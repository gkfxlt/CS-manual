...
namespace kaanh
{
	...
	auto sixRrobot::executeRT(PlanTarget &target)->int
	{
		//读取末端位姿（位置 + 四元素）
		double PqEnd[7];
	...
	sixRrobot::sixRrobot(const std::string &name) :Plan(name)
	{
		command().loadXmlStr(
			"<Command name=\"sixRrobot\">"
			"</Command>");
	}
	...
	auto createPlanRootRokaeXB4()->std::unique_ptr<aris::plan::PlanRoot>
	{
		...
		plan_root->planPool().add<kaanh::sixRrobot>();
